  ...
  async function fetchAll() {
    ...
    try {
      const [{ data: m }, { data: r }] = await Promise.all([
        supabase.from('admin_users').select('*, admin_roles(id, name, description)').order('created_at'),
          ...
          {roles.map(r => {
            ...
            return (
              <div key={r.id} style={{ ...S.roleCard, borderTop: `3px solid ${c}` }}>
                ...
                <div style={{ color: '#94a3b8', fontSize: 12, marginTop: 4 }}>{r.name}</div>
              </div>
            )
          })}
        </div>
                ...
                {members.map(m => {
                  ...
                  return (
                    <tr key={m.id} style={{ ...S.tr, opacity: m.is_active ? 1 : 0.5 }}>
                      <td style={S.td}>
                        <div style={{ color: '#f1f5f9', fontWeight: 700 }}>{m.display_name}</div>
                      ...
                      <td style={S.td}>
                        <button onClick={() => toggleActive(m)} style={{ ...S.actionBtn, color: m.is_active ? '#ef4444' : '#10b981', borderColor: m.is_active ? '#ef444444' : '#10b98144' }}>
          ...
          <div style={S.modal} onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleAddAdmin} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                ...
                <input type="email" required placeholder="user@email.com" value={newEmail} onChange={e => setNewEmail(e.target.value)} style={S.input} />
              </div>
              <div>
