  ...
  async function submitAction() {
    ...
    try {
      ...
      if (modal.action !== 'note') {
        ...
        // Ban/suspend evasion check: fingerprint their current photos so a
        // new signup re-uploading the same photo gets caught (hash comparison
        // only, not face-matching). Best-effort — don't fail the ban action
        // if this call has trouble.
        if (modal.action === 'ban' || modal.action === 'suspend') {
          try {
            const { data: { session } } = await supabase.auth.getSession();
            await fetch('/api/hash-user-photos', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + session?.access_token },
              body: JSON.stringify({ userId }),
            });
          } catch (hashErr) {
            console.error('[hash-user-photos] request failed', hashErr.message);
          }
        }
      }
