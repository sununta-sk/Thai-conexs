...
export default function AnnouncementsPage() {
  ...
  async function loadList() {
    setLoad(true)
    try { const { data } = await supabase.from('announcements').select('*').order('created_at',{ascending:false}); setList(data||[]) } catch {}
    setLoad(false)
  }
  ...
  async function save() {
    ...
    try {
      ...
      await loadList(); setF({title:'',body:'',type:'info',active:true}); setForm(false); showT('✓ Published')
    } catch { showT('Error','error') }
    setSaving(false)
  }

  async function toggleActive(item) {
    try { await supabase.from('announcements').update({active:!item.active}).eq('id',item.id); setList(prev=>prev.map(a=>a.id===item.id?{...a,active:!a.active}:a)) } catch {}
  }

  async function del(id) {
    if (!confirm('ลบประกาศนี้?')) return
    try { await supabase.from('announcements').delete().eq('id',id); setList(prev=>prev.filter(a=>a.id!==id)) } catch {}
  }
      ...
      <div style={S.page}>
        ...
        {showForm && (
          <div style={S.formCard}>
            <div style={S.field}><label style={S.lbl}>Title</label><input value={form.title} onChange={e=>setF(p=>({...p,title:e.target.value}))} style={S.input} placeholder="Title..."/></div>
            <div style={S.field}><label style={S.lbl}>Message</label><textarea value={form.body} onChange={e=>setF(p=>({...p,body:e.target.value}))} rows={3} style={S.textarea}/></div>
