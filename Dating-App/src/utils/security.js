...
export function assertNoServiceRoleKey() {
  ...
  export function injectCSP() {
    ...
    const policy = [
      ...
      `connect-src 'self' https://${supabaseHost} wss://${supabaseHost} https://*.supabase.co wss://*.supabase.co https://thai-conexs-production.up.railway.app wss://thai-conexs-production.up.railway.app https://api.anthropic.com`,
      `frame-src 'none'`,
      `object-src 'none'`,
      `base-uri 'self'`,
      `form-action 'self'`,
